...
def start_bot(message):
    ...
    bot.send_message(user, 'Привет мой друг,хочешь проверить свое знание,тогда пиши /test')
    print(user)
...
def messagemap(message):
    ...
    button5 = types.InlineKeyboardButton("Тест2", callback_data='5')
...
def callback_inline(call): #функция для кнопок, тут вы перехватываете нажатие кнопок
  try:
    if call.message:
        ...
        if call.data == '1':
            bot.send_message(call.message.chat.id, 'Тут тест про основы ООП, Если хотите пройти тeст основы ООП, нажмите на Тест')
        if call.data == '2':
        ...
        if call.data == '5':
            bot.send_message(call.message.chat.id, 'https://itproger.com/test/python')
        if call.data == '6':
